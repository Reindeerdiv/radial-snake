...
+┊   ┊120┊    let loadsize = 0;
+┊   ┊121┊
+┊   ┊122┊    // We use the "after" method because we want the following callback to be invoked
+┊   ┊123┊    // only once all assets are loaded
+┊   ┊124┊    let onload = _.after(loadsize, () => {
+┊   ┊125┊      delete this.screen.loading;
+┊   ┊126┊      callback();
+┊   ┊127┊    });
+┊   ┊128┊
+┊   ┊129┊    // This object can load assets
+┊   ┊130┊    let assetsLoader = new Engine.AssetsLoader(() => {
+┊   ┊131┊      loadsize++;
+┊   ┊132┊      return () => onload();
+┊   ┊133┊    });
+┊   ┊134┊
+┊   ┊135┊    // The "load" method returns the assets loaded by the screen
+┊   ┊136┊    let screenAssets = this.screen.load(assetsLoader);
+┊   ┊137┊    // The returned assets will be available on screen's assets object
+┊   ┊138┊    _.extend(this.screen.assets, screenAssets);
+┊   ┊139┊  }
+┊   ┊140┊
+┊   ┊141┊  // Disposes screen assets
+┊   ┊142┊  unloadScreen() {
+┊   ┊143┊    let assetsNames = this.screen.unload && this.screen.unload();
+┊   ┊144┊    _.omit(this.assets, assetsNames);
+┊   ┊145┊  }
+┊   ┊146┊
 ┊ 95┊147┊  // Defines global assets
 ┊ 96┊148┊  extendAssets(assets) {
 ┊ 97┊149┊    _.extend(this.assets, assets);
```
[}]: #
